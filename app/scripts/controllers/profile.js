...
angular.module('tracklistmeApp')
    .controller('ProfileCtrl', function($scope, $http, $timeout, $auth, $alert, Account, FileUploader, CONFIG) {
        ...
        $scope.importArtistFromBeatport = function() {
            $http.get('http://tracklist.me/API/parseArtists.php').success(function(data) {

                for (var i = data.length - 1; i >= 0; i--) {
                    console.log(data[i]);

                    $scope.importImage(data[i].name, data[i].image)
                };
        ...
        $scope.importImage = function(name, image) {
