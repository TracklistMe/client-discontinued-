 ...
 */
app.controller('CartCtrl', function($location, $scope, $state, $auth,
  $stateParams, $http, CONFIG, ngCart, stripe) {
    ...
    return stripe.card.createToken($scope.payment.card)
      ...
      .catch(function(err) {
        if (err.type && /^Stripe/.test(err.type)) {
          ...
          console.log('Other error occurred, possibly our API', err.message);
        }
      });
  };
});
