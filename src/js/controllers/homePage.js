 ...
 */
app.controller('HomepageController', function($sce, $scope, $http, CONFIG) {

  $scope.releases = [];
  $scope.serverURL = CONFIG.url;

  // TODO (bortignon@): should be merged with the underneath function 
  ...
  $scope.addRelease = function(id) {
    ...
    for (var i = 0; i < release.Tracks.length; i++) {
      ...
      var track = {};
      track.title = release.Tracks[i].title +
        ' (' + release.Tracks[i].version + ')';
      ...
      // Mp3 version 
      track.sources.push({
        src: $sce.trustAsResourceUrl($scope.serverURL +
          '/snippets/' + release.Tracks[i].snippetPath),
        type: 'audio/mpeg'
      }, {
        src: $sce.trustAsResourceUrl($scope.serverURL +
          '/snippets/' + release.Tracks[i].oggSnippetPath),
        type: 'audio/ogg'
      });
      tracksObject.push(track);
    }

    $scope.$emit('addNewTrack', tracksObject);
  };
